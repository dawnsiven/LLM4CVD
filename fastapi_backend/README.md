...
用途：获取当前 FastAPI 进程中维护的全部任务列表。

说明：

- 返回值是数组
- 按任务创建时间倒序排列
- 只包含当前这个 FastAPI 进程启动后创建的任务
...
- 任务状态保存在内存中，FastAPI 服务重启后不会保留历史任务
- `GET /api/jobs` 只能看到当前服务进程启动后提交的任务
- 日志接口只返回日志尾部文本，不做文件下载
- 输出文件目前只返回路径，没有单独提供下载路由
