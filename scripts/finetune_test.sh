...
if [ $# -lt 6 ]; then
    echo "Usage: $0 <DATASET_NAME> <RESULT_MODEL_NAME> <LLM_MODEL_NAME> <LENGTH> <POS_RATIO> <BATCH_SIZE> [LENGTH_BUCKET] [CUDA]"
    exit 1
fi
...
TOKENIZERS_PARALLELISM=false CUDA_VISIBLE_DEVICES="${CUDA}" python LLM/finetuning_test.py \
    ...
    >"${OUTPUT_DIR}/finetuning_${MODEL_NAME}_lora_${RESULT_MODEL_NAME}_imbalance_${DATASET_TAG}.log"
