...
PRIMARY_OUTPUT_DIR="outputs/${MODEL_NAME}_imbalance/${DATASET_NAME}_${LENGTH}_${POS_RATIO}"
FALLBACK_OUTPUT_DIR="outputs/${MODEL_NAME}_imbalance_test/${DATASET_NAME}_${LENGTH}_${POS_RATIO}"

if [[ -d "${PRIMARY_OUTPUT_DIR}" ]]; then
    OUTPUT_DIR="${PRIMARY_OUTPUT_DIR}"
elif [[ -d "${FALLBACK_OUTPUT_DIR}" ]]; then
    OUTPUT_DIR="${FALLBACK_OUTPUT_DIR}"
else
    echo "No existing output directory found."
